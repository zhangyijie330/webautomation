...
public class YYS004_Repay_Business extends TestDriver {
	...
	public void test() {
		...
		try {
			...
			// 获取所有的还款日期和期数
			List<Map<String, String>> repayPlanList = DbUtil
					.queryDataList(
							"select distinct * from product_temp,product_repay_plan where product_temp.product_name=product_repay_plan.name and stage != '募集期' and product_temp.product_id='"
									+ productID + "'", DbType.Local);
			String repayDate;
			String nextRepayDate;
			// 还款起始期数，指定从顺序的第几期开始跑
			int stageNo = 1;
			for (int i = stageNo - 1; i < repayPlanList.size(); i++) {
				if (i < repayPlanList.size() - 1) {
					repayDate = repayPlanList.get(i).get("repay_date");
					nextRepayDate = repayPlanList.get(i + 1).get("repay_date");
					System.out.println("stage=[" + i + "]");
					boolean byStage = this.toRepayBusiness(repayDate,
							nextRepayDate, i);
					if (byStage == false) {
						result = false;
					}
					...
					repayDate = repayPlanList.get(i).get("repay_date");
					boolean byStage = this.toRepayBusiness(repayDate, "", i);
					if (byStage == false) {
						result = false;
					}
				}
	 ...
	 * @throws Exception
	 */
	public boolean toRepayBusiness(String repayDate, String nextRepayDate,
		...
		keyWords = new KeyWords(driver, log);
		String productID = "YYS004";
		// 要查找的项目名称
		...
		// 项目还款日期
		// repayDate = repayService.getRepayDate(projectName);
		System.out.println("项目还款日期为[" + repayDate + "]");
		// 获取当前服务器时间
		String currTime = SSHUtil.sshCurrentTime(log);
		System.out.println("当前服务器时间为：" + currTime);
		// 修改服务器时间到还款日
		...
		while (true) {
			...
			if (repayService.isFindProject(keyWords, projectName)) {
				...
				int repayTableListSize = repayTableList.size();
				// 校验当期还款列表内容的期望值
				if (repayService.checkRepayTableRecords(projectName, repayDate,
						repayTableList)) {
				...
				// 切换到当期还款明细签页
				keyWords.click(OrUtil.getBy("repay_detail_curr_coupon_xpath",
						ObjectLib.XhhObjectLib));
				...
				if (repayService.checkRepayProgress(keyWords, projectName)) {
					log.info("【还款进度校验】成功");
					// 当还款期是最后1期回写用户投资表的投资状态
					if (nextRepayDate.equals("")) {
						repayService.updateInvest(projectName, repayDate,
								"已还款结束");

					} else {
						// 当还款期非最后1期回写用户投资表的投资状态及更新下一个还款时间
						repayService.updateInvest(projectName, repayDate, "还款中"
								+ repayService.getRepayPorgress(projectName));
						repayService.updateInverstRD(projectName,
								nextRepayDate, repayDate);
					}
					// 回写用户账户信息表和用户资金记录表
					repayService.updateAcct(projectName, repayDate);
					// 回写保理账户资金流表
					repayService.updateFund(projectName, repayDate);
				} else {
					log.error("【还款进度校验】失败");
					result = false;
					throw new Exception("【还款进度校验】失败！");
				}
				// 恢复服务器时间
				SSHUtil.sshRecoverTime(log);
				// xhh-3958:还款后，进入保理公司访问【资金记录】页面点击“资金记录”
				...
				// xhh-3959:还款后，保理公司账户校验资金流水校验，检验资金记录表
				...
				// xhh-3762:case1.访问鑫合汇主站，用户登入鑫合汇主站进行账户数据检验
				...
				if (keyWords.isElementExist(By
					...
					if (curPageNum < toltalPageNum) {
						...
						// 点击下一页按钮
						keyWords.click(OrUtil.getBy("repay_next_page_xpath",
								ObjectLib.XhhObjectLib));
						ThreadUtil.sleep(3);
					} else {
	...
	/**
	 * 功能： xhh-3764:case3鑫合汇主站登录成功以后查看账户信息
	 * 
	 * @param repayDate
	 * @param uid
	 * @param userName
	...
	public boolean checkUser(KeyWords kw, String productCode, String uid,
		...
		if (pubXhhService.login(kw, userName, pwd)) {
			...
			// xhh-3768:用户查看【理财记录】数据,获取理财记录信息
			...
			if (accountService.checkRowCount2(kw, financeRowCount)) {
