...
/**
 * 用户投资用例：月益升，3天募集期，每月等额本息
 * 
 * @author sunlingyun
 * 
 */
public class YYS004_UserInvest extends TestDriver {
				...
				if (publicService.login(kw, user, pwd)) {
					...
					if (financeService.findProductByPaging(kw, productCode)) {
						...
						if (financeService.checkMapInProDetail(kw,
								...
								checkAcctBalance, checkProfit,
								RepayWay.permonth, discountType)) {
						...
						if (financeService.checkRepayPlan(kw, productCode)) {
							log.info("【还款计划】正常还款日期与合计数据校验成功");
						} else {
							log.error("【还款计划】正常还款日期与合计数据校验失败");
							bFinalResult = false;
						}
						...
						financeService.setAmountAndPWd(kw, investAmount + "",
								paymentPwd);
						// 计算获得预计收益
						// String dateUsed = expectProdInfo.get("days_used");
						// String annualRate =
						// expectProdInfo.get("annual_rate");
						// investEnd = expectProdInfo.get("invest_end_date");
						...
						if (financeService.payConfirmAndCheckSuccess(kw)) {
							...
							log.error("用户投资失败");
							bFinalResult = false;
							throw new Exception("用户投资失败");
						}
						// 投资成功后校验信息
						financeService.checkInfoInIvstSuccPage(kw,
								expectProdInfo.get("end_bid_time"),
								expectProdInfo.get("start_profit_date"), true,
								expectProdInfo.get("repay_date_stage1"), true);
						/*
						 * 回写需要修改的数据
						 */
						...
						// 校验条数
						if (accountService.checkRowCount(kw, 2)) {
						 ...
						 */
						// 校验条数
						if (accountService.checkRowCount(kw, 3)) {
