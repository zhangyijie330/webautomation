...
/**
 * 用户投资用例：月益升，3天募集期，到期还付本息
 * 
 * @author sunlingyun
 * 
 */
public class YYS008_UserInvest extends TestDriver {

	public static KeyWords kw;
	public static boolean bFinalResult = true;
	private String productNumber = "YYS008";
			...
			for (int i = startUserNo - 1; i < lst.size(); i++) {
				...
				if (publicService.login(kw, user, pwd)) {
					...
					if (financeService.findProductByPaging(kw, productCode)) {
						 ...
						 */
						Map<String, String> expectAccountMap = AccountDao
								.getUserAcctByUid(uid);
						if (accountService.checkAccountInvestVal(kw,
								expectAccountMap)) {
							log.info("【账户总览】页面字段校验成功");
						...
						// 访问我的奖励并校验满减券、红包、加息券,只有u17与u18用户可用
						if (i == 0 || i == 1) {

							if (accountService.viewAndCheckCouponList(kw,
									expectAccountMap)) {
								log.info("【我的奖励】校验成功");
							} else {
								log.error("【我的奖励】校验失败");
