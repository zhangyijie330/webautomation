...
/**
 * RYS001项目支付：日益升，一天募集期，到期还本付息
 * 
 * @author zhangyijie
 * 
 */

public class RYS001_Payment extends TestDriver {
	...
	public void Test() {
		...
		String productid = "RYS001";
		String bmpuserName = BaseConfigUtil.getBMPName();// 业务系统登录账号
		String bmppwd = BaseConfigUtil.getBMPPwd();// 业务系统登录密码
		String bluserName = BaseConfigUtil.getFactorName();// 前台系统登录账号
		String blpwd = BaseConfigUtil.getFactorPwd();// 前台系统登录密码
		...
		try {
			...
			int fundrecordcount = Integer.parseInt(Loan.getloantemp(productid)
					.get("fundrecord_count"));// 保理账户资金记录对应条数
