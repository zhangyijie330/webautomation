...
public class YYS008_PayMent extends TestDriver {
		...
		try {
			...
			for (int i = 0; i < lst.size(); i++) {
				...
				List<Map<String, String>> userrepayplan = DbUtil.queryDataList(
						...
						DbType.Local);
				Map<String, String> uaccount = DbUtil.querySingleData(
						"select * from user_acct where uid='" + uid + "'",
						DbType.Local);
				DecimalFormat fm = new DecimalFormat("###,##0.00");
				String reward = fm.format(Double.parseDouble(uaccount
						.get("coupons_used"))
						+ Double.parseDouble(uaccount.get("bonus_used")));
				String date2 = userrepayplan.get(0).get("repay_date");
				String capital = Loan.formatStr(userrepayplan.get(0).get(
						"capital"));
				double sumprofit = 0;
				...
				String reminder;
				if (reward.equals("0.00")) {
					reminder = "投资" + productCode + "将收到" + capital + "元本金 和"
							+ profit + "元收益 ，预计16:00-24:00到账。";
				} else {
					reminder = "投资" + productCode + "将收到" + capital + "元本金 ( 含"
							+ reward + "元奖励) 和" + profit
							+ "元收益 ，预计16:00-24:00到账。";
				}
