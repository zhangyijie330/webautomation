 ...
 * @return void
 */
public class YYS005_Repay_Business extends TestDriver {
	...
	public void test() {
		...
		try {
			...
			// for (Map<String, String> map : repayPlanList) {
			// String repayDate = map.get("repay_date");
			// String stage = map.get("stage");
			// System.out.println("stage=[" + stage + "]");
			// this.toRepayBusiness(repayDate,stage);
			// }
	...
	public boolean toRepayBusiness(String repayDate, String nextRepayDate,
		...
		while (true) {
			...
			if (repayService.isFindProject(keyWords, projectName)) {
				...
				/*
				 * // 切换到还款情况签页 keyWords.click(OrUtil.getBy(
				 * "repay_detail_repay_situation_xpath",
				 * ObjectLib.XhhObjectLib)); keyWords.waitPageLoad(); boolean
				 * tmpres = keyWords.isElementVisible(By .xpath(
				 * "//div[@id='ajaxContent_Application_ProjectRepayment_repaymentList']/div/table/thead/tr[1]/th[text()='期次']"
				 * )); System.out.println("判断期次显示："+tmpres);
				 */
				...
				if (repayService.checkRepayProgress(keyWords, projectName)) {
					...
					if (nextRepayDate.equals("")) {
						...
						repayService
								.updateInvest(projectName, repayDate, "还款中");
				...
				for (int i = 0; i < incomeExpenseTableList.size(); i++) {
					log.info("i=" + i + "-->"
							+ StringUtils.format(incomeExpenseTableList.get(i)));
				}
				// 检验资金记录表
	...
	/**
	 * 功能： xhh-3764:case3鑫合汇主站登录以后查看账户信息
	 * 
	 * @param repayDate
	 * @param uid
	 * @param userName
