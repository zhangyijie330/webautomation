...
/**
 * 用户投资用例：月益升，3天募集期，到期还付本息，展期4天
 * 
 * @author sunlingyun
 * 
 */
public class YYS003_UserInvest extends TestDriver {
			...
			for (int i = startUserNo - 1; i < lst.size(); i++) {
				...
				if (publicService.login(kw, user, pwd)) {
					...
					if (financeService.findProductByPaging(kw, productCode)) {
						...
						// 还款计划描述信息校验
						String extendDays = expectProdInfo.get("extend_time");
						String expDesc = "允许最长" + extendDays + "天展期";
						...
						if (financeService.checkRepayPlan(kw, productCode)) {
							log.info("【还款计划】正常还款日期与合计校验成功");
						} else {
							log.error("【还款计划】正常还款日期与合计校验失败");
							bFinalResult = false;
						}
