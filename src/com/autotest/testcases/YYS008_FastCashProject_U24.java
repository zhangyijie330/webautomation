...
/**
 * U24 用户投资的速兑通项目再次变现变现操作
 * 
 * u24 发起变现操作 xhh-7256 ~ xhh-7263，变现后账户查看xhh-7264 ~ xhh-7272
 * 
 * @author 002194
 * 
 */
public class YYS008_FastCashProject_U24 extends TestDriver {
	public static KeyWords	kw;
	public static boolean	result;
	private static String	className		= "YYS008_FastCashProject_U24 ";
	private static String	productId		= "SDT006";
	private static String	userNumber		= "u24";
	private static String	product_id		= "SDT007";
	...
	public void test() {
		...
		try {
			...
			fastcash.changTime2(end_bid_time, 193);
			...
			if (!StringUtils.isEquals(expectedMap, actualMap, log)) {
				log.error(className + userName + "变现页面校验失败");
				log.error(ScreenShotUtil.takeScreenshot(ThreadName, kw.driver));
				result = false;
			} else {
				log.info(className + userName + "变现页面校验成功");
			}

			// 获取变现页面上的相关值
			log.info(className + " 获取变现页面上的相关值");
			Map<String, String> castFastValue = fastcash.CreatFastCashPage(kw);

			// 提交变现数据
			log.info(className + " 提交变现数据");
			...
			if (userAccountService.checkAccountInvestVal(kw, expextAccountMap)) {
				log.info(userNumber + uid + " 【账户总览】页面字段校验成功");
			} else {
				log.error(userNumber + uid + " 【账户总览】页面字段校验失败");
				result = false;
			}
